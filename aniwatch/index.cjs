...
__export(index_exports, {
  HiAnime: () => hianime_exports,
  HiAnimeError: () => HiAnimeError
});
...
var client = import_axios.default.create(clientConfig);
...
var log = (0, import_pino.pino)(loggerOptions);
...
async function getHomePage() {
  ...
  try {
    ...
    const $ = (0, import_cheerio.load)(mainPage.data);
...
async function getAZList(sortOption, page) {
  ...
  try {
    ...
    const $ = (0, import_cheerio2.load)(resp.data);
...
async function getGenreAnime(genreName, page) {
  ...
  try {
    ...
    const $ = (0, import_cheerio3.load)(mainPage.data);
...
async function getAnimeQtipInfo(animeId) {
  ...
  try {
    ...
    const $ = (0, import_cheerio4.load)(mainPage.data);
...
async function getAnimeEpisodes(animeId) {
  ...
  try {
    ...
    const $ = (0, import_cheerio5.load)(episodesAjax.data.html);
...
async function getAnimeCategory(category, page) {
  ...
  try {
    ...
    const $ = (0, import_cheerio6.load)(mainPage.data);
    const selector = "#main-content .tab-content .film_list-wrap .flw-item";
...
async function getProducerAnimes(producerName, page) {
  ...
  try {
    ...
    const $ = (0, import_cheerio7.load)(mainPage.data);
...
async function getEpisodeServers(episodeId) {
  ...
  try {
    ...
    const $ = (0, import_cheerio8.load)(data.html);
...
async function getAnimeAboutInfo(animeId) {
  ...
  try {
    ...
    const $ = (0, import_cheerio9.load)(mainPage.data);
    try {
...
async function _getAnimeSearchResults(q, page = 1, filters) {
  try {
    ...
    const $ = (0, import_cheerio10.load)(mainPage.data);
    const selector = "#main-content .tab-content .film_list-wrap .flw-item";
    res.hasNextPage = $(".pagination > li").length > 0 ? $(".pagination li.active").length > 0 ? $(".pagination > li").last().hasClass("active") ? false : true : false : false;
    res.totalPages = Number(
      $('.pagination > .page-item a[title="Last"]')?.attr("href")?.split("=").pop() ?? $('.pagination > .page-item a[title="Next"]')?.attr("href")?.split("=").pop() ?? $(".pagination > .page-item.active a")?.text()?.trim()
...
var StreamSB = class {
  ...
  async extract(videoUrl, isAlt = false) {
    ...
    const m3u8_urls = await import_axios3.default.get(res.data.stream_data.file, {
      headers
    });
...
var StreamTape = class {
  ...
  async extract(videoUrl) {
    try {
      const { data } = await import_axios4.default.get(videoUrl.href).catch(() => {
        throw new Error("Video not found");
      });
      const $ = (0, import_cheerio11.load)(data);
...
var RapidCloud = class {
  ...
  async extract(videoUrl) {
    ...
    try {
      ...
      let res = null;
      res = await import_axios5.default.get(
      ...
      let decryptKey = await (await import_axios5.default.get(
      ...
      if (!decryptKey) {
        decryptKey = await (await import_axios5.default.get(
          "https://raw.githubusercontent.com/cinemaxhq/keys/e1/key"
        )).data;
      }
      ...
      try {
        if (encrypted) {
          ...
          const decrypt = import_crypto_js.default.AES.decrypt(sources, decryptKey);
          sources = JSON.parse(decrypt.toString(import_crypto_js.default.enc.Utf8));
        }
      } catch (err) {
      ...
      if (videoUrl.href.includes(new URL(this.host).host)) {
        ...
        for (const source of sources) {
          const { data } = await import_axios5.default.get(source.file, options);
...
var crypto = import_crypto.webcrypto;
var wasm;
...
function isDetached(buffer) {
  if (buffer.byteLength === 0) {
    const formatted = import_util.default.format(buffer);
...
var Z = (z2, Q0) => {
  try {
    var Q1 = import_crypto_js2.default.AES.decrypt(z2, Q0);
    return JSON.parse(Q1.toString(import_crypto_js2.default.enc.Utf8));
  } catch (Q2) {
  }
  return [];
};
...
var M = (a, P) => {
  try {
    var Q0 = import_crypto_js2.default.AES.decrypt(a, P);
    return JSON.parse(Q0.toString(import_crypto_js2.default.enc.Utf8));
  } catch (Q1) {
    log.info(Q1.message);
  }
  return [];
...
var MegaCloud = class {
  ...
  async extract(videoUrl) {
    try {
      ...
      const { data: srcsData } = await import_axios6.default.get(
      ...
      let text;
      const { data } = await import_axios6.default.get(
  ...
  decrypt(encrypted, keyOrSecret, maybe_iv) {
    ...
    if (maybe_iv) {
      ...
      for (let i2 = 0; i2 < 3; i2++) {
        md5Hashes[i2] = import_crypto2.default.createHash("md5").update(digest).digest();
  ...
  async extract3(embedIframeURL) {
    try {
      ...
      const { data: rawSourceData } = await import_axios6.default.get(megacloudUrl);
      const encrypted = rawSourceData?.sources;
      if (!encrypted)
        throw new Error("Encrypted source missing in response");
      const decrypted = import_crypto_js3.default.AES.decrypt(encrypted, key).toString(
        import_crypto_js3.default.enc.Utf8
      );
...
async function _getAnimeEpisodeSources(episodeId, server = "hd-1" /* VidStreaming */, category = "sub") {
  ...
  try {
    ...
    const $ = (0, import_cheerio12.load)(resp.data.html);
    let serverId = null;
    try {
...
async function getAnimeEpisodeSources(episodeId, server, category) {
  try {
    ...
    const [episodeSrcData, animeSrc] = await Promise.all([
      ...
      import_axios7.default.get(animeURL, {
        headers: {
    ...
    const $ = (0, import_cheerio12.load)(animeSrc?.data);
    try {
      anilistID = Number(
        JSON.parse($("body")?.find("#syncData")?.text())?.anilist_id
      );
...
async function getAnimeSearchSuggestion(q) {
  try {
    ...
    const $ = (0, import_cheerio13.load)(data.html);
...
async function getEstimatedSchedule(date, tzOffset = -330) {
  ...
  try {
    ...
    const $ = (0, import_cheerio14.load)(mainPage?.data?.html);
    const selector = "li";
...
async function getNextEpisodeSchedule(animeId) {
  ...
  try {
    ...
    const $ = (0, import_cheerio14.load)(mainPage.data);
